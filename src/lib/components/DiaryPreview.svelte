<script>
  import { staggerChildren, fadeIn, textReveal } from '$lib/utils/transitionUtils';
  import { onMount } from 'svelte';
  import { getDiaryPreview } from '$lib/utils/pdfGenerator';
  
  // Month and year for the diary
  export let month;
  export let year;
  ...
  onMount(() => {
    // Delay before showing the component (for entrance animations)
    setTimeout(() => {
      visible = true;
    }, delay);
  });
...
<div class="diary-preview">
  ...
  {#if visible}
    <div class="diary-pages" use:staggerChildren={{ staggerTime: 100, transition: fadeIn, duration: 300 }}>
      {#each previewItems as item, i}
        <div class="diary-page">
          <div class="day-header">
            <span class="day-number">{item.day}</span>
            <span class="day-name">
              {new Date(year, month - 1, item.day).toLocaleString('default', { weekday: 'long' })}
            </span>
          </div>
