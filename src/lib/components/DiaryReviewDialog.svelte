...
<div class="flex flex-col h-full text-gray-200 p-6 max-w-4xl mx-auto">
  <div class="flex justify-between items-start pb-6 border-b border-gray-700 mb-6">
    <div class="flex-1">
      ...
      <p class="text-gray-400 m-0 text-sm">Step {currentStep + 1} of {totalSteps}: {stepInfo.title}</p>
    </div>
    <button 
      on:click={cancel} 
    ...
    <div class="flex justify-between mb-2">
      {#each Array(totalSteps) as _, i}
        <div 
          ...
          role="button"
        >
          <div class="w-10 h-10 rounded-full flex items-center justify-center text-white font-medium text-sm mb-1 
            {i < currentStep ? 'bg-indigo-700' : i === currentStep ? 'bg-indigo-500' : 'bg-gray-700'}">
            {i < currentStep ? '✓' : i + 1}
          </div>
          <span class="text-xs text-center max-w-[80px] 
            {i === currentStep ? 'text-indigo-300 font-medium' : 'text-gray-400'}">
        ...
        {#if i < totalSteps - 1}
          <div class="flex-1 flex items-center px-2">
            <div class="h-0.5 w-full {i < currentStep ? 'bg-indigo-700' : 'bg-gray-700'}"></div>
          </div>
        {/if}
      {/each}
    </div>
