...
import { getDaysInMonth, getMonthOptions } from './dateUtils.js';
 ...
 */
function generateQuestions(month, year) {
 ...
 */
export function generateDiaryPDF(month, year, userData = {}) {
  const doc = new jsPDF();
  ...
  doc.text(`${userName}'s Personal Reflection Journal`, pageWidth / 2, 30, { align: 'center' });
  
  // Introduction
  doc.setFontSize(12);
  doc.setTextColor(80, 80, 80);
  const intro = `This diary contains daily reflection questions for ${monthName} ${year}. ` +
 ...
 */
export function getDiaryPreview(month, year) {
