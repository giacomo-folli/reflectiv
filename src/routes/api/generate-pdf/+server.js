 ...
 */
function generateDiary(month, year, userData) {
  const doc = new jsPDF({
  ...
  doc.setFontSize(12);
  doc.text(`Generated on ${DateTime.now().toFormat('MMMM d, yyyy')}`, 105, 270, { align: 'center' });
  
  // Select questions for this month
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    doc.setFontSize(10);
    doc.text(`Page ${day + 1} of ${daysInMonth + 1}`, 105, 287, { align: 'center' });
  }
...
export async function GET({ url, locals }) {
  ...
  return new Response(pdfData, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="Reflection_Diary_${monthName}_${year}.pdf"`
    }
  });
}